...
import { Tag, Plus, Edit2, Trash2, Check, X, Settings, Info, ShoppingBag } from 'lucide-react';
...
interface PlatformManagerModalProps {
  ...
  onRenamePlatform: (oldName: string, newName: string) => void;
  onDeletePlatform: (platformName: string) => void;
}

export const PlatformManagerModal: React.FC<PlatformManagerModalProps> = ({
  isOpen,
  onClose,
  platforms,
  coins,
  ...
  // Calculate coin count listed on each platform
  const getCoinCount = (platformName: string) => {
    return coins.filter(c => c.listingPlatform === platformName).length;
  };
  ...
  const startEditing = (platformName: string) => {
    setEditingPlatform(platformName);
    setEditValue(platformName);
  };
  ...
  const saveRename = (oldName: string) => {
    ...
    if (trimmed && trimmed !== oldName) {
      onRenamePlatform(oldName, trimmed);
    }
    setEditingPlatform(null);
    setEditValue('');
  };
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-serif font-bold text-slate-100">
                Verkaufsplattformen verwalten
              </h2>
              <p className="text-xs text-slate-400">
                Verkaufskanäle wie Ricardo, eBay, Tutti, Auktionen verwalten
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          ...
          <form onSubmit={handleAddSubmit} className="space-y-2">
            <label className="block text-xs font-semibold text-slate-300">
              ➕ Neue Verkaufsplattform hinzufügen
            </label>
            <div className="flex space-x-2">
              <input
                ...
                placeholder="z.B. Ricardo.ch, eBay.de, Tutti.ch, Auktionshaus"
          ...
          <div className="space-y-2">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 border-b border-slate-800 pb-1.5 flex justify-between">
              <span>Bestehende Plattformen ({platforms.length})</span>
            ...
            {platforms.length === 0 ? (
              <div className="text-center py-8 text-slate-500 text-xs bg-slate-900/50 rounded-xl border border-slate-800/80">
                Keine eigenen Plattformen angelegt. Füge oben deinen ersten Verkaufskanal hinzu!
              </div>
            ) : (
              <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
                {platforms.map(platformName => {
                  const count = getCoinCount(platformName);
                  const isEditing = editingPlatform === platformName;
                  const isDeleting = confirmDeletePlatform === platformName;
                      ...
                      {isDeleting ? (
                        <div className="flex items-center justify-between w-full bg-rose-950/40 p-1.5 rounded-lg border border-rose-500/40 text-xs">
                          <span className="text-rose-200 font-medium">
                            "{platformName}" wirklich löschen? {count > 0 ? `(${count} Münzen betroffen)` : ''}
                          </span>
                        ...
                        <div className="flex items-center space-x-2 flex-1 mr-2">
                          <input
                            ...
                            onKeyDown={e => {
                              if (e.key === 'Enter') saveRename(platformName);
                          ...
                          />
                          <button
                            onClick={() => saveRename(platformName)}
                            title="Speichern"
                        ...
                        <>
                          <div className="flex items-center space-x-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0">
                              ...
                              <span className="text-[11px] text-slate-400">
                                {count} {count === 1 ? 'Münze eingestellt' : 'Münzen eingestellt'}
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center space-x-1 shrink-0">
                            <button
                              onClick={() => startEditing(platformName)}
                              title="Plattform umbenennen (aktualisiert alle zugeordneten Münzen)"
                              className="p-2 text-slate-400 hover:text-amber-300 hover:bg-amber-500/10 rounded-lg transition-colors"
                            >
                              <Edit2 className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => setConfirmDeletePlatform(platformName)}
          ...
          <div className="p-3.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs text-blue-200 flex items-start space-x-2.5">
            ...
            <div>
              <strong>Tipp:</strong> Sie können Ihre Plattformen völlig frei benennen (z.B. <em>"Ricardo.ch"</em>, <em>"eBay Shop"</em> oder <em>"Lokal-Auktion"</em>). Beim Erfassen oder Bearbeiten einer Münze wählen Sie einfach die entsprechende Plattform aus.
            </div>
          </div>
        </div>
